...
This demo demonstrates cellular jamming detection on UniRTOS, including parameter configuration, event-driven status reporting, and result interpretation.

- Demonstrates registering a jamming detection event callback using `qosa_event_notify_register`
- Demonstrates querying and printing default jamming detection parameters (mode, RSSI, RSRP, RSRQ, period, shake period)
- Demonstrates enabling jamming detection mode and URC reporting via `qosa_nw_set_jamm_detect_param`
- Demonstrates applying detection configuration with `qosa_nw_set_jamm_detect_func`
- Demonstrates handling detection status events with result interpretation (0: no jammed / 1: no jamming / 2: jammed)
